...
--
-- Hôte : 127.0.0.1:3306
-- Généré le :  lun. 20 mai 2019 à 19:17
...
DROP PROCEDURE IF EXISTS `drop_friend_request`$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_game_state` (IN `p_game` INT)  NO SQL
BEGIN

SELECT state from game where id = p_game;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_player` (IN `p_pseudo` VARCHAR(30))  NO SQL
BEGIN

SELECT * from player where pseudo=p_pseudo;
...
DROP PROCEDURE IF EXISTS `get_players_position`$$
...
DROP PROCEDURE IF EXISTS `get_spectate_game`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_spectate_game` ()  NO SQL
BEGIN 

SELECT * from game where private=0;
    
END$$

DROP PROCEDURE IF EXISTS `get_square`$$
...
DROP PROCEDURE IF EXISTS `get_unstart_game`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_unstart_game` ()  NO SQL
BEGIN 

SELECT * from game where state='create';
...
DROP PROCEDURE IF EXISTS `player_leave`$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `start_game` (IN `p_game` INT)  NO SQL
BEGIN 
...
DELETE from invitation where game=p_game;


END$$

DROP PROCEDURE IF EXISTS `update_account`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_account` (IN `p_mail` VARCHAR(150), IN `p_pass` VARCHAR(200), IN `p_pseudo` VARCHAR(30))  NO SQL
BEGIN
...
(172, 3, 180, 0, 'play', 'admin'),
(171, 3, 180, 1, 'play', 'dongle'),
...
('dongle', 'dongle@gmail.com', 0, NULL, '6a01bfa30172639e770a6aacb78a3ed4'),
('admin2', 'admin2@gmail.com', 0, NULL, '21232f297a57a5a743894a0e4a801fc3'),
('admin', 'admin@gmail.com', 0, NULL, '21232f297a57a5a743894a0e4a801fc3'),
('dongle', 'dongle@gmail.com', 0, NULL, '6a01bfa30172639e770a6aacb78a3ed4');
...
(14, 'admin2', 157, 6, 6, 2, 0, 0, 0, 0, NULL, NULL),
